...
class Reconstruction(object):
    ...
    def __init__(self, params):
        ...
        self.proj = np.zeros([self.nView, self.nv, self.nu], dtype=np.float32)
        self.image = np.zeros([self.nz, self.ny, self.nx], dtype=np.float32)
    ...
    def Filter(N, pixel_size, FilterType, cutoff):
        ...
        if FilterType == 'ram-lak':
            ...
            filter = filter * (0.54 + 0.46 * (cos(w / cutoff + eps)))
        elif FilterType == 'hann':
            filter = filter * (0.5 + 0.5 * cos(w / cutoff + eps))
